...
exec > >(tee -a /tmp/stvcl_debug.txt) 2>&1
set -x
...
if [ ! -d /usr/lib64 ]; then
	...
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/stvcl
fi
...
wget --no-check-certificate 'https://github.com/Belfagor2005/S.T.V.C.L-/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'stvcl--main/usr' '/'
set +e
cd
...
IMAGE VERSION: $distro_version" >> /tmp/stvcl_debug.txt
sleep 5
killall -9 enigma2
exit 0
